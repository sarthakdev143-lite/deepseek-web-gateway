...
function buildTools() {
  return [
    ...
    '- replace_in_file(path, find, replace, [all]) — surgical edit to existing file',
    ...
    '- run_command(command, [timeout_ms]) — execute shell command (sandboxed)',
    '- start_server(name, command, port) — start background server with health check',
    '- stop_server(name) — stop a background server',
