...
function buildSystemPrompt() {
  ...
  const lines = [
    ...
    'HOW TO CALL TOOLS',
    '─────────────────',
    'When you need to use a tool, your response must contain one or more fenced code',
    'blocks tagged "tool_call":',
    '',
    FENCE + 'tool_call',
    '{',
    '  "name": "TOOL_NAME_HERE",',
    ...
    '',
    'CRITICAL RULES:',
    '- You may write natural language explanations or strategy comments before or after the tool call blocks.',
    '- If the task is NOT 100% complete, you MUST include at least one tool call block in your response.',
    '- You may include MULTIPLE tool_call blocks in ONE response to execute actions in batch (e.g. reading or writing/modifying several files at once). This is highly recommended to perform changes faster.',
    '  Example of writing multiple files in one response:',
    FENCE + 'tool_call',
    '{ "name": "write_file", "args": { "path": "src/a.js", "content": "..." } }',
    FENCE,
    FENCE + 'tool_call',
    '{ "name": "write_file", "args": { "path": "src/b.js", "content": "..." } }',
    FENCE,
    '- For writing or modifying multiple files, you can also use a single "write_files" tool call.',
    '- Every tool call block must be valid JSON with exactly "name" and "args" keys.',
    '- After receiving tool result(s), call another tool OR give your final response if done.',
    '- Only write plain prose (no tool_call block) when the task is 100% complete.',
    '',
    'WHEN TO STOP',
    '────────────',
