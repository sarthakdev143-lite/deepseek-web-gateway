...
function buildSystemPrompt() {
  ...
  const lines = [
    ...
    '',
    'CRITICAL RULES:',
    '- Output ONLY tool_call block(s) — no prose, no greeting, nothing else.',
    '- You may include MULTIPLE tool_call blocks in ONE response ONLY when the calls',
    '  are completely independent reads (e.g. reading several files at once).',
    '  Example of valid multi-block response:',
    FENCE + 'tool_call',
    '{ "name": "read_file", "args": { "path": "src/a.js" } }',
    FENCE,
    FENCE + 'tool_call',
    '{ "name": "read_file", "args": { "path": "src/b.js" } }',
    FENCE,
    '- For writes and commands, always use a single tool call per response.',
    '- Content must be valid JSON with exactly "name" and "args" keys.',
    '- After receiving tool result(s), call another tool OR give your final response.',
